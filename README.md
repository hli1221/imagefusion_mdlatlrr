...
The frames in first row and second row are selected from 'car10' and 'car41' (VOT-RGBT 2019), respectively. First three columns are the results of LADCF. And the last three columns are the tracking results of GFSDCF. the \emph{rgb} and \emph{infrared} denote the input of trackers is just one modality data (RGB or infrared). The \emph{level-1} to \emph{level-4} demonstrate that the input of trackers is the fused frames which are generated by MDLatLRR.
